...
import { categoriesHeader } from '../../../../Components/TextsLanguages';
...
function CategoriesItem() {
    ...
    const {id, category, language='ru'} = useParams()
    ...
    function showNumber(e){
      e.target.textContent = '+998 71 233 80 10'
    }
        ...
        <div id="image__box">
          <div className="row">
            ...
            <div className="col-lg-6 image__box-texts">
                <button type='button'> <img src={telegram} alt="logo" /> <Link to={"/"+(language ? language : 'ru') + "/about"}>{(language=='ru' ? "Оставить заявку" : language=='uz' ? 'Ariza qoldirish' : 'Application')} </Link></button>
                <button type='button' onClick={(e)=>showNumber(e)}> {(language=='ru' ? 'ПОКАЗАТЬ ТЕЛЕФОН' : language=='uz' ? 'Nomerni ko`rsatish' :'View number')} </button>
                <div className="image__box-texts-content">
                  <div className='image__box-texts-content-item'><img src={vector1} alt="logo"/> <p>{(language=='ru' ? 'Правила безопасной сделки' : language=='uz' ? 'Xavfsizlik bitimi qoidalari' : 'Safe Deal Rules')} </p></div>
                  <div className='image__box-texts-content-item'><img src={vector2} alt="logo"/><p>{(language=='ru' ? 'Вернуться в категорию' : language=='uz' ?  'Kategoriyaga qaytish' : 'Back to category')}</p> <Link to={"/"+(language ? language : 'ru') + "/categories/"+id}>{names.name}</Link></div>
                  <div className='image__box-texts-content-item'><img src={vector3} alt="logo"/><p>{(language=='ru' ? 'Дата последнего обновления:' : language=='uz' ? 'Oxirgi yangilanish sanasi:' : 'Last update:')  } {categorieItem.updated}</p></div>
                </div>
            </div>
          </div>
          <p>{categorieItem.description}</p>
        </div>
        <h5>{(language=='ru' ? 'Похожие предложения от “CSS Comfort Cooling Service” OOO' : language=='uz' ? '“CSS Comfort Cooling Service” OOO dan o`xshash takliflar' : 'Similar offers from “CSS Comfort Cooling Service” OOO')}</h5>
      {
